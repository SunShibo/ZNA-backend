<template>
    <div class="table">
        ...
        <div class="container">
            ...
            <el-table v-loading="$store.state.requestLoading" :data="data" border class="table" ref="multipleTable"
                ...
                <el-table-column prop="lgDTO" :formatter="lgDTORole" label="语言权限">
                </el-table-column>
...
<script>
    export default {
        ...
        methods: {
            lgDTORole(val){//格式化语言权限
                if(!val)return;
                let str = '';
                for (let i = 0, len = val.lgDTO.length; i < len; i++) {
                    str += i != len - 1 ? val.lgDTO[i].languageName + ', ' : val.lgDTO[i].languageName;
                }
                return str;
            },
            getLanguageData(){//获取所有语言
                this.$axios({
                    url: '/Order/roleLanguage',
                    method: 'POST'
                }).then(res => {
                    if (res) {
            ...
            formatLanguage(languageArr){//格式化语言权限ID
                //将之前的置空
                this.languageIds = [];
                for (let i = 0, len = languageArr.length; i < len; i++) {
                    this.languageIds.push(languageArr[i].id);
                }
            },
            getMenuData() {
            ...
            handleEdit(index, row) {
                ...
                this.formatLanguage(item.lgDTO);
                this.form = {
                ...
                this.$nextTick(() => {
                    ...
                    this.$refs.languageTree.setCheckedKeys(this.languageIds);
                })
            },
            ...
            saveEdit() {
                var self = this;
                // this.form.des = this.$refs.tree.getCheckedNodes().map(v=>{
                //     return v.label+" , ";
                // });
                this.form.menuIds = this.$refs.tree.getCheckedKeys();
                this.form.languageId = this.$refs.languageTree.getCheckedKeys();
                var newlist = [];
