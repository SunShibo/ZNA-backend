<template>
    <div class="login-wrap">
        <div class="ms-login">
            <div class="ms-title">ZNA官网后台管理系统</div>
                ...
                this.$axios.post('/admin/adminLogin', {
                    ...
                    if (res.success) {
                        ...
                        localStorage.setItem('ms_u', JSON.stringify({  //保存用户信息
                            ...
                            phoneNumber: res.data.admin.mobile,
                            roleName: res.data.admin.roleName
                        }));
                        //保存菜单
                        sessionStorage.setItem('me_u', JSON.stringify(res.data.menu));
                        this.$router.push({name: res.data.menu[0].children[0].index});//跳转
                    }
                }).catch((err) => {
                    console.error(err);
                })
...
<style scoped>
    .login-wrap {
        ...
        width: 100%;
        height: 100%;
        background: url(../../assets/img/3c9bcbc0-15dc-4a6f-a81f-5112936b7773.jpg) no-repeat;
